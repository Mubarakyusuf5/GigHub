...
import { formatNaira, handlePayment } from "../../Data";
...
export const PostJobModal = ({ onClose, fetchJobs }) => {
  const [formData, setFormData] = useState({
   ...
   const [platformFeePercentage, setPlatformFeePercentage] = useState(3) //fetch from backend
  ...
  useEffect(() => {
    if (formData.category) {
      setSkills(
        categories.find((cat) => cat.category === formData.category)?.skills ||
          []
      );
    }
...
const totalAmount = Number.parseFloat(formData.budget) + platformFee
    ...
    try {
      ...
      // handlePayment(totalAmount, formData.title, clientemail, clientname)
      fetchJobs();
      onClose();
    } catch (error) {
      toast.error(
                ...
                {formData.budget && (
                  <div className="mt-2 p-3 bg-blue-50 rounded-lg">
                    ...
                    <p className="text-sm text-gray-700 mt-1">
                      ...
                      {formData.budget
                        ? formatNaira(
                            Number.parseFloat(formData.budget) + platformFee
                          )
                        : "₦0.00"}
                    </p>
                  </div>
            ...
            {formData.category && (
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Search and select skills from the list or press Enter to add
                  custom skills
                </p>
              </div>
            )}
