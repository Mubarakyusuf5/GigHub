...
export const TestDetails = ({ onClose, fetchJobs }) => {
  ...
  const calculatePlatformFee = (budgetAmount) => {
    if (!budgetAmount || isNaN(budgetAmount)) return 0
    return (Number.parseFloat(budgetAmount) * platformFeePercentage) / 100
  }
                ...
                {formData.budget && !isNaN(formData.budget) && Number.parseFloat(formData.budget) > 0 && (
                  <div className="mt-2 p-3 bg-blue-50 rounded-lg">
                    <p className="text-sm text-gray-700">
                      ...
                      {calculatePlatformFee(formData.budget).toLocaleString()}
                    </p>
